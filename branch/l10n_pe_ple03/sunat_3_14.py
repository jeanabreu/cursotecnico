...
class ple_3_14 (osv.Model):
    _name = "l10n_pe.ple_3_14"
    _inherit = "l10n_pe.ple"

    _columns= {
        'lines_ids': fields.one2many ('l10n_pe.ple_line_3_14', 'ple_3_14_id', 'Lines', readonly=True, states={'draft':[('readonly',False)],}),
    }

    def action_reload (self, cr, uid, ids, context=None):
        assert ids and len(ids)==1
        ple_line_obj = self.pool.get('l10n_pe.ple_line_3_14')
        ...
        move_lines = self.get_move_lines (cr, uid, period_ids, '3_14', ple.company_id.id, context=context)
    ...
    def action_report (self, cr, uid, ids, context=None):
        ...
        return {
            ...
            'report_name': 'l10n_pe.sunat_3_14',
            'datas': {},
        }
    ...
    def get_output_filename (self, cr, uid, ids, context=None):
        return "sunat_3_14.txt"

class ple_line_3_14 (osv.Model):
    _name = 'l10n_pe.ple_line_3_14'
    ...
    _columns = {
        'ple_3_14_id': fields.many2one('l10n_pe.ple_3_14', 'PLE', on_delete='cascade'),
        'state': fields.related ('ple_3_14_id', 'state', string='State', type="char"),
        'company_id': fields.related ('ple_3_14_id', 'company_id', string='Company', type="many2one", relation="res.company"),
        'partner_id': fields.many2one('res.partner', 'Partner', on_delete="restrict"),

        'partner_doc_type_3': fields.selection(_get_doc_type_selection, "Doc. type", required=True, size=3, help="Tipo de Documento de Identidad del trabajador"),
        'partner_doc_number_4': fields.char("Partner's doc. number", size=15, help="Número de Documento de Identidad del trabajador"),
        'partner_name_5': fields.char("Partner's name", size=60, required=True, help="Apellidos y Nombres del trabajador"),
        'amount_6': fields.float('Amount', digits=(12,2), help="Saldo Final"),
...
'9' cuando la operación corresponde a un periodo anterior y SI ha sido anotada en dicho periodo."""),
    ...
    def onchange_partner_id (self, cr, uid, ids, partner_id, period_id, context=None):
        vals = {}
        if partner_id:
            ple_3_14_obj = self.pool.get('l10n_pe.ple_3_14')
            period_obj = self.pool.get('account.period')
            partner_obj = self.pool.get('res.partner')
            
            lines = ple_3_14_obj.get_move_lines(cr, uid,
                                                ple_3_14_obj.get_all_periods_up_to(period_id, context=context),
                                                '3_14',
                                                partner_id=partner_id,
                                                context=context)
...
class ple_configuration (osv.osv):
    ...
    def get_report_type (self, cr, uid, context=None):
        ...
        rep_types.append (('3_14', '3.14 Beneficios sociales de los Trabajadores'))
