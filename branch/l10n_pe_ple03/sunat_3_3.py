...
class ple_3_3 (osv.Model):
    _name = "l10n_pe.ple_3_3"
    _inherit = "l10n_pe.ple"

    _columns= {
        'lines_ids': fields.one2many ('l10n_pe.ple_line_3_3', 'ple_3_3_id', 'Lines', readonly=True, states={'draft':[('readonly',False)],}),
    }

    def action_reload (self, cr, uid, ids, context=None):
        assert ids and len(ids)==1
        ple_line_obj = self.pool.get('l10n_pe.ple_line_3_3')
        ...
        move_lines = self.get_move_lines (cr, uid, period_ids, '3_3', ple.company_id.id, context=context)
    ...
    def get_output_filename (self, cr, uid, ids, context=None):
        return "sunat_3_3.txt"

class ple_line_3_3 (osv.Model):
    _name = 'l10n_pe.ple_line_3_3'
    ...
    _columns = {
        'ple_3_3_id': fields.many2one('l10n_pe.ple_3_3', 'PLE', on_delete='cascade'),
        'state': fields.related ('ple_3_3_id', 'state', string='State', type="char"),
        'company_id': fields.related ('ple_3_3_id', 'company_id', string='Company', type="many2one", relation="res.company"),
        ...
        'partner_name_5': fields.char("Partner's name", size=60, required=True, help="Apellidos y Nombres, Denominación o Razón Social del cliente"),
        'operation_date_6': fields.date ('Operation date', required=True, help="Fecha de emisión del Comprobante de Pago"),
        'amount_7': fields.float('Amount', digits=(12,2), help="Monto de cada cuenta por cobrar del cliente"),
...
'9' cuando la operación corresponde a un periodo anterior y SI ha sido anotada en dicho periodo."""),
    ...
    def onchange_partner_id (self, cr, uid, ids, partner_id, period_id, context=None):
        vals = {}
        if partner_id:
            ple_3_3_obj = self.pool.get('l10n_pe.ple_3_3')
            period_obj = self.pool.get('account.period')
            partner_obj = self.pool.get('res.partner')
            
            lines = ple_3_3_obj.get_move_lines(cr, uid,
                                                ple_3_3_obj.get_all_periods_up_to(period_id, context=context),
                                                '3_3',
                                                partner_id=partner_id,
                                                context=context)
...
class ple_configuration (osv.osv):
    ...
    def get_report_type (self, cr, uid, context=None):
        ...
        rep_types.append (('3_3', '3.3 Clientes'))
