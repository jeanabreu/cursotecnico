...
class account_expense(osv.Model):
    ...
    _columns = {
            ...
            'voucher_ids': fields.one2many('account.voucher','expense_id','Vouchers', readonly=True),
        ...
        for expense in self.browse(cr, uid, ids, context=context):
            for expense_invoice in expense.invoice_ids:
                if expense_invoice.state in ('draft','proforma','proforma2'):
                    if expense.period_id and not expense_invoice.period_id:
                        self.pool.get('account.expense.invoice').write(cr, uid, [expense_invoice.id],{'period_id':expense.period_id.id},context=context)
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        for expense in self.browse(cr, uid, ids, context=context):
            if expense.state in ('paid','done'):
                voucher_obj.cancel_voucher(cr, uid, [v.id for v in expense.voucher_ids], context=context)
            for expense_invoice in expense.invoice_ids:
    ...
    def action_cancel_draft(self, cr, uid, ids, *args):
        ...
        for expense in self.browse(cr, uid, ids):
            if expense.voucher_ids:
                voucher_obj.unlink(cr, uid, [v.id for v in expense.voucher_ids])
            for expense_invoice in expense.invoice_ids:
    ...
    def onchange_journal(self, cr, uid, ids, journal_id, exchange_rate, context=None):
        ...
        if journal.currency.id and journal.company_id.currency_id.id <> journal.currency.id:
            res['value']['exchange_rate'] = journal.currency.rate or journal.company_id.currency_id.rate or 0.0
        else:
