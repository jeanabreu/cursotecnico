...
class res_currency(osv.osv):
    ...
    _columns = {
        ...
        'update_from': fields.date('Update Rates From', help="Used to update the rates from this date in batch"),
        'update_time': fields.selection([("%02d.%02d" %(hours, minutes), "%d.%d hs" %(hours, minutes))  for hours in range(0,24) for minutes in [0, 15, 30, 45]], 'Update time'),
...
#             cron_obj.write(cr, uid, cron_ids, cron_descriptor, context=context)
    ...
    def rate_add(self, cr, uid, ids, rate_type_id, value, date=None, context=None):
        rcr_obj = self.pool.get('res.currency.rate')
        today = date or fields.date.context_today(self, cr, uid, context=context)
